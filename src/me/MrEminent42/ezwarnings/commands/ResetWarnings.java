...
public class ResetWarnings implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if (cmd.getName().equalsIgnoreCase("setwarnings")) {
			...
			if (args.length == 2) {
				if (sender.hasPermission("warnings.reset.warnings")) {
					...
					*/
					...
					try {
						...
						EzWarnings.messages.sendTranslation(sender, "resetwarnings.usage", true);
					}
					
					WarningManager.setWarnings(Bukkit.getOfflinePlayer(args[0]), w);
					EzWarnings.messages.sendTranslation(sender, "resetwarnings.message", true, new Variable("{player}", args[0]));
				} else {
